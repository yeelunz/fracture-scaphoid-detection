...
def sort_indices(vertices: torch.Tensor, mask: torch.Tensor):
    """Sort vertices in clockwise order using angles.
    ...
    Returns:
        sorted_index: LongTensor (B, N, 9)
    
    Note:
        why 9? the polygon has maximal 8 vertices. +1 to duplicate the first element.
        the index should have following structure:
            (A, B, C, ... , A, 0, 0, 0) 
        and 0 indicates padding where necessary.
    """
    B, N, V, _ = vertices.shape
    num_valid = torch.sum(mask, dim=2)  # (B, N)

    # Compute mean for angle calculation
    mean = torch.sum(vertices * mask.unsqueeze(-1), dim=2, keepdim=True) / num_valid.unsqueeze(-1).unsqueeze(-1)
    vertices_normalized = vertices - mean  # (B, N, 24, 2)
    
    # Compute angles
    angles = torch.atan2(vertices_normalized[..., 1], vertices_normalized[..., 0])  # (B, N, 24)
    
    # Assign a large angle to masked out vertices
    angles = angles + (1.0 - mask.float()) * 1e6
    
    # Sort angles
    sorted_angles, sorted_indices = torch.sort(angles, dim=2)
    
    # Select first 8 indices and duplicate the first to make 9
    topk = 8
    sorted_indices = sorted_indices[:, :, :topk]
    first_indices = sorted_indices[:, :, :1]
    sorted_indices = torch.cat([sorted_indices, first_indices], dim=2)  # (B, N, 9)
